...
```
先获取该bean配置的所有property。依次调用bean factory中各个post processor的postProcessAfterInstantiation方法，然后依次调用bean factory中各个post processor的postProcessPropertyValues方法。如果我们设置了**<context:annotation-config />**，则会在当前bean对应的类中查找带有@Autowired注解的field， 然后加载field对应的bean，然后进行注入操作。
